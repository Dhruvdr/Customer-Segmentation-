...
try :
    ...
    df = pd.read_excel("d:\DEVELOPMENT\PROJECTS\Customer Segmentation\Online Retail.xlsx")
...
# print("--- Starting CLV Calculation ---")

# Creating specialized CLV DataFrame
# Observation period end is set to the max date
...
# Filtered out customers who bought ones (Gamma-Gamma requires repeat buyers)
clv_summary = clv_summary[clv_summary['frequency'] > 0]

# Fiting the BG/NBD Model (Predicts Future Frequency)
bgf = BetaGeoFitter(penalizer_coef=0.01)
bgf.fit(clv_summary['frequency'], clv_summary['recency'], clv_summary['T'])

# No.of purchases each customer will make in the next 30 days
clv_summary['predicted_purchases_30_days'] = bgf.conditional_expected_number_of_purchases_up_to_time(
    30, clv_summary['frequency'], clv_summary['recency'], clv_summary['T']
)

# Fiting the Gamma-Gamma Model (Predicts Future Monetary Value)
ggf = GammaGammaFitter(penalizer_coef=0.01)
ggf.fit(clv_summary['frequency'], clv_summary['monetary_value'])

# Calculating the final 12-Month Customer Lifetime Value
clv_summary['12_Month_CLV'] = ggf.customer_lifetime_value(
    bgf,
...
# print(clv_summary[['predicted_purchases_30_days', '12_Month_CLV']].head())
...
# plt.figure(figsize=(12,6))
# sns.countplot(x=rfm_df['Loyalty'], palette='Set2')
# plt.title('Customer Distribution by RFM Loyalty Segment')
...
rfm_df['Recency'] = np.where(rfm_df['Recency'] <= 0, 1, rfm_df['Recency'])
rfm_df['Monetary'] = np.where(rfm_df['Monetary'] <= 0, 1, rfm_df['Monetary'])
rfm_df['Frequency'] = np.where(rfm_df['Frequency'] <= 0, 1, rfm_df['Frequency'])
...
rfm_df['Recency_log'] = np.log(rfm_df['Recency']).round(3)
rfm_df['Frequency_log'] = np.log(rfm_df['Frequency']).round(3)
rfm_df['Monetary_log'] = np.log(rfm_df['Monetary']).round(3)
# print(rfm_df)
...
# Merging RFM_df and CLV dataframe to get the final dataframe for analysis
final_df = rfm_df.merge(clv_summary[['predicted_purchases_30_days', '12_Month_CLV']], 
                        left_index=True, right_index=True, how='left')

# Fill NaN values (customers who didn't have enough data for CLV predictions) with 0
final_df['12_Month_CLV'].fillna(0, inplace=True)
final_df['predicted_purchases_30_days'].fillna(0, inplace=True)
# print(final_df.head(10))
 
# Aggregating CLV by Segments
clv_by_segment = final_df.groupby('Loyalty').agg({'12_Month_CLV': ['mean', 'sum', 'count'], 'predicted_purchases_30_days': 'mean'}).reset_index()
clv_by_segment.columns = ['Loyalty', 'Average_CLV', 'Total_CLV', 'Customer_Count','Predicted_Purchases']
# print("Future Value by Layalty Segments:")
# print(clv_by_segment) 

# Aggregating CLV by K-Means Clusters
cluster_clv_summary = final_df.reset_index().groupby('Cluster_based_rec_freq_mon').agg({
    'Recency': 'mean',
    'Frequency': 'mean',
    'Monetary': 'mean',
    '12_Month_CLV': ['mean', 'sum'], # Average per person, and Total Value of the cluster
    'CustomerID': 'count'
})
cluster_clv_summary.columns = ['Avg_Recency', 'Avg_Frequency', 'Avg_Monetary', 'Avg_CLV', 'Total_CLV', 'Customer_Count']
# print("Future Value by K-Means Cluster:")
# print(cluster_clv_summary)

# Spread of CLV across Clusters
# plt.figure(figsize=(10,6))
# sns.boxplot(x='Cluster_based_rec_freq_mon', y='12_Month_CLV', data=final_df[final_df['12_Month_CLV']>0])
# plt.title('Predicted 12-Month CLV by K-Means Cluster')
# plt.ylim(0,10000) # Limit y-axis for better visualization
# plt.show()

# Total expected revenue by loyalty tier
# plt.figure(figsize=(10, 6))
# clv_sum_by_loyalty = final_df.groupby('Loyalty')['12_Month_CLV'].sum().reset_index()
# sns.barplot(x='Loyalty', y='12_Month_CLV', data=clv_sum_by_loyalty, palette='viridis')
# plt.title('Total Expected Revenue in Next 12 Months by Loyalty Tier')
# plt.ylabel('Total Expected Revenue (£)')
# plt.show()

# Identifyiing "at-risk" Whales
# Probability alive 
clv_summary['probability_alive'] = bgf.conditional_probability_alive(
    clv_summary['frequency'], clv_summary['recency'], clv_summary['T']
)
final_df['probability_alive'] = final_df.index.map(clv_summary['probability_alive'])
final_df['probability_alive'].fillna(0, inplace=True) 

# "at risk whales": Top tier customers who have less than 50% chance of returning
at_risk_whales = final_df[(final_df['Loyalty'] == 'A') & (final_df['probability_alive'] < 0.5)]

print(f"Number of high value customers at risk of churning: {len(at_risk_whales)}")
print(f"Send these specific customers a highly targeted discount campaign immediately!")
